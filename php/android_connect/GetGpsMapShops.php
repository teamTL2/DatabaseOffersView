...
class GetGpsMapShops {
    ...
    public function get_Gps_Shops(){
        ...
        mysqli_stmt_prepare($stmt,"SELECT Shop_ID , ShopName , Longitude , Latitude FROM shops WHERE ( ? >= Longitude AND Longitude >= ? AND ? >= Latitude AND Latitude >= ?)");
        mysqli_stmt_bind_param($stmt,"dddd", $this->_Ymax, $this->_Ymin, $this->_Xmax, $this->_Xmin);
        mysqli_stmt_execute($stmt);

        $stmt->bind_result($_SESSION['Shop_ID'], $_SESSION['ShopName'], $_SESSION['Longitude'], $_SESSION['Latitude']);
        ...
        while (mysqli_stmt_fetch($stmt)) {
            ...
            $Shop["Longitude"] =$_SESSION['Longitude'];
            $Shop["Latitude"] =$_SESSION['Latitude'];
...
$newListShops = new GetGpsMapShops();
